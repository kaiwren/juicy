		...
		function() {
			...
			for(var i = 0; i < specs.length; i++) {
				...
				for(var j = 0; j < spec.examples.length; j++) {
					...
					sb.push('				<pre id="example_'+specs[i].examples[j].id+'_code" class="examples-code"><code>'+JSSpec.util.escapeTags(example.target.toString())+'</code></pre>');
					sb.push('			</li>');
				}
				sb.push('		</ul>');
				sb.push('	</li>');
...
JSSpec.Logger.prototype.onExampleEnd = function(example) {
	var li = document.getElementById("example_" + example.id);
    var exampleCode = document.getElementById("example_"+example.id+"_code");
    ...
    else if(JSSpec.options.autocollapseCode){
...
window.onload = function() {
	if(JSSpec.specs.length > 0) {
		...
		var list = document.getElementById('list') || document.createElement('div');
